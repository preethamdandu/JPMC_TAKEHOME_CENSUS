...
def main():
    ...
    pipe = Pipeline([("preprocess", preprocessor), ("classifier", XGBClassifier(random_state=args.random_state, scale_pos_weight=scale_pos_weight, eval_metric="logloss"))])
    if args.no_grid:
        pipe.fit(X_train, y_train)
        best_pipe = pipe
    else:
        grid = GridSearchCV(pipe, {"classifier__n_estimators": [100, 200], "classifier__max_depth": [4, 6, 8], "classifier__learning_rate": [0.05, 0.1]}, cv=StratifiedKFold(5, shuffle=True, random_state=args.random_state), scoring="roc_auc", n_jobs=-1, refit=True, verbose=1)
    ...
    with open(os.path.join(out_dir, "classifier_xgb_model.pkl"), "wb") as f:
        pickle.dump(best_pipe, f)
    with open(os.path.join(out_dir, "classifier_xgb_metrics.txt"), "w") as f:
        f.write("Test set metrics (XGBoost):\n")
        for k, v in metrics.items(): f.write(f"  {k}: {v:.4f}\n")
    print("Saved: outputs/classifier_xgb_model.pkl, classifier_xgb_metrics.txt")
