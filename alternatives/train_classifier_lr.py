...
def main():
    ...
    pipe = Pipeline([
        ...
        ("classifier", LogisticRegression(random_state=args.random_state, class_weight="balanced", max_iter=1000, solver="lbfgs")),
    ])
    if args.no_grid:
        pipe.fit(X_train, y_train)
        best_pipe = pipe
    else:
        param_grid = {"classifier__C": [0.01, 0.1, 1.0, 10.0], "classifier__max_iter": [1000]}
    ...
    y_pred = (y_proba >= args.threshold).astype(np.int64)
    ...
    with open(os.path.join(out_dir, "classifier_lr_model.pkl"), "wb") as f:
        pickle.dump(best_pipe, f)
    with open(os.path.join(out_dir, "classifier_lr_metrics.txt"), "w") as f:
        f.write("Test set metrics (Logistic Regression):\n")
        for k, v in metrics.items():
            f.write(f"  {k}: {v:.4f}\n")
    print(f"Saved: {out_dir}/classifier_lr_*")
